...
让 HR 无需配置技术参数，即可完成“列表 → 详情 → 原文 → 确认或纠错”。

### 6.2 Inputs

- CandidateSnapshot 和 ReviewCandidate 固定样例；
- 列表、详情、原文和 HRReviewCommand API；
- 状态、推荐等级、维度证据、缺失和异常定义；
- 管理员与 HR 的身份边界；
- 合成候选人和脱敏页面数据。

### 6.3 Owned capabilities
...
- BOSS 自动化和飞书鉴权实现；
- 自动淘汰、约面、外联、阶段推进和写回；
- 用静态文案掩盖列表、详情或复核路径的数据不一致。

### 6.5 Implementation order

1. 使用固定 ReviewCandidate 样例实现列表；
2. 实现候选人详情和维度证据；
3. 实现原文定位；
4. 实现 HRReviewCommand 提交、版本冲突和幂等反馈；
5. 实现管理员连接状态；
6. 补齐空数据和异常状态；
7. 接入 SCREENING_AND_JD_ASSETS 的真实接口；
8. 验证完整 HR 用户路径。

### 6.6 Completion criteria

- HR 身份不显示凭证、Scope、CLI 或模型配置；
- 列表和详情使用同一 candidate_id 与 revision；
- 页面展示服务端评分，不重新计算；
- 原文证据可以定位或明确显示无法定位；
- 确认和纠错带操作者、原因和版本；
- 重复提交不会重复写入，过期 revision 有明确提示；
- 权限不足、解析失败、模型失败和空队列都有独立状态；
- 页面没有自动淘汰、约面、写回或“AI 已决定”动作。
...
WORKSPACE_ROOT=<路径>，BASELINE=<提交或版本>。
负责人是新源。只完成 UI 层，先使用固定 ReviewCandidate 样例打通 HR 路径，再接 SCREENING_AND_JD_ASSETS 接口。完成后生成 RETURN_REPORT.md。
~~~
